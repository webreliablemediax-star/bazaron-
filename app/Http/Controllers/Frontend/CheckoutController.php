...
class CheckoutController extends Controller
{
    ...
    
    {
        ...
        return redirect()->route('home')
            ->with('error', 'Admins cannot access checkout');
    }
    ...
    public function complete(Request $request)
    {
        ...
        if (in_array(auth()->user()->user_type, ['admin'])) {
        return redirect()->route('home')
            ->with('error', 'Admin cannot buy products. Please register as a customer to continue.');
    }
...
if ($request->payment_method == 'cod') {
    ...
    foreach ($carts as $cart) {
        ...
        $vendor = VendorProfile::where(
            'id',
            $cart->product_variation->product->vendor_id
        )->first();

        if ($vendor && $vendor->has_own_logistics == 1) {

            flash('COD not available for this seller')->error();
            return back();
        }
    }
}
...
// 🔥 COD VALIDATION END
    ...
    flash('Product is out of stock. Try later.')->error();
    return back();
}
                }else{ 
...
$orderGroup->billing_address_id  = $request->billing_address_id;
            ...
            $grandTotal = getSubTotal($carts, false, '', false); // GST Included
$taxAmount  = getTotalTax($carts);

$orderGroup->sub_total_amount = $grandTotal - $taxAmount;
$orderGroup->total_tax_amount = $taxAmount;
            ...
            $orderGroup->total_shipping_cost = $logisticZone ? $logisticZone->standard_delivery_charge : 0;
...
$rate = $currentCurrency ? $currentCurrency->rate : 1;

$orderGroup->total_tips_amount = $request->tips / $rate;
...
$orderGroup->grand_total_amount =
    $grandTotal
    + $orderGroup->total_shipping_cost
    + $orderGroup->total_tips_amount
    - $orderGroup->total_coupon_discount_amount;
            ...
            $firstProduct = $carts[0]->product_variation->product;

$order->vendor_id = $firstProduct->vendor_id;
$order->product_id = $firstProduct->id;
            ...
            // 🔥 INVOICE LOGIC START
$vendor = VendorProfile::where('user_id', $order->vendor_id)->first();

if ($vendor) {

    $next = $vendor->invoice_last_number + 1;

    // update counter
    $vendor->invoice_last_number = $next;
    $vendor->save();

    // generate invoice
    $invoice_no = $vendor->invoice_prefix . '-' . str_pad($next, 4, '0', STR_PAD_LEFT);
